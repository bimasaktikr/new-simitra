...
<div class="container mx-auto p-4 dark:bg-gray-900 dark:text-gray-200">
    <h1 class="text-3xl font-bold mb-4 text-gray-900 dark:text-gray-100">Daftar Tipe Pembayaran</h1>

    <div class="mt-6">
        <div class="flex justify-between mb-4">
            <div class="relative w-1/3">
                <input type="text" id="search-payment-type" class="block w-full p-2 pl-10 text-sm border rounded-lg border-gray-300 bg-white shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200" placeholder="Search..." />
            ...
            <div class="flex space-x-4">
                <button onclick="window.location='{{ route('addpaymenttype') }}'" class="inline-flex items-center px-4 py-2 text-white bg-orange-500 border border-transparent rounded-lg shadow-sm hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 dark:bg-orange-600 dark:hover:bg-orange-700">
                    Tambah Tipe Pembayaran
                </button>
            </div>
        </div>

        <div id="survey-payment-type" class="bg-gray-100 p-4 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
            @include('payment_type.table')
        </div>
        ...
        <form action="{{ route('survei') }}" method="GET">
            <div class="flex justify-between mt-4 items-center">
                ...
                <div>
                    {{ $paymentTypes->appends(['per_page' => request()->get('per_page', 10)])->links('components.pagination') }}
                </div>
            </div>
        </form>
